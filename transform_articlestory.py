#!/usr/bin/env python3
"""
openscience.works ArticleStory UX Transformer
Applies 6 improvements to an ArticleStory HTML file:
  ...
  5. Sub-tabs inside "Who is citing this work?"
  6. Paginated citations table (rows extracted to JS, 25 per page)
"""
...
SRC = Path("/sessions/loving-great-brown/mnt/openscienceworks/articlestory_10.1056_NEJMoa0810696.html")
DST = Path("/sessions/loving-great-brown/mnt/openscienceworks/articlestory_10.1056_NEJMoa0810696_new.html")
...
for s in soup.find_all("section"):
    h2 = s.find("h2")
    if h2 and "Citations" in h2.get_text() and "who" not in h2.get_text().lower():
        cit_section = s
        break
...
# Keyed on lowercase substring of badge text
BADGE_ENHANCEMENTS = {
...
# For discipline-badge divs (OA type + field)
        ...
        for span in badge_div.find_all("span", recursive=False):
            ...
            for badge_key, enh in BADGE_ENHANCEMENTS.items():
                if badge_key in raw_text:
                    ...
                    # Append small gray source suffix
                    sfx = bs_soup.new_tag(
                        "span",
                        style=(
    ...
    for db in hero_el.find_all("div", class_="discipline-badge"):
        ...
        for disc_key, enh in DISCIPLINE_ENHANCEMENTS.items():
            if disc_key in key:
                ...
                label_str    = enh["label"]
                # Keep the SVG icon if present, rebuild text
                svg = db.find("svg")
                db.clear()
                if svg:
                    db.append(svg)
...
# scite_bar_block intentionally omitted — full chart stays in Who's Citing
impact_sec = soup.find(id="sec-impact")
if impact_sec:
    h2_im = impact_sec.find("h2")
    if h2_im:
        # Remove all siblings after h2 (the old plain metric-list bullets)
        for _el in list(h2_im.find_next_siblings()):
            _el.extract()
        # Append the new rich block
